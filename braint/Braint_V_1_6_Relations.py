...
class RelationsFrame():
    def __init__(self, parent, rootTk, manager):
        print 'relations frame version 1.6'
    ...
    def create_treering(self, parent_frame):
        ...
        self.ren_win_treering = self.treering_plot.get_render_window()
        self.render_widget_treering = vtkTkRenderWindowInteractor(parent_frame,rw=self.ren_win_treering, width=700, height=700)

        self.render_widget_treering.grid(row=0, column=0, sticky = N+W+E+S)

        self.treering_plot.init_render()
    ...
    def button_apply_handler(self):
        params = self.list_items_to_add.get(0, END)
        self.destroy_frame()
        self.parent.update_frame_with_visualization(params)

    def destroy_frame(self):
        self.ren_win_treering.Finalize()
        del self.ren_win_treering
        self.render_widget_treering.destroy()
        self.relations_frame.destroy()
    ...
    def create_button_apply(self, parent_frame):
        button_add=tk.Button(parent_frame,text='Apply', command = self.button_apply_handler)
