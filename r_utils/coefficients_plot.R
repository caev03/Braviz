#' Plots the coefficients of an lm fit
...
#' @param order order coefficients per estimate value. Beware of coefficient scales to avoid misleading results and consider standardizing (e.g. using \code{\link{arm::standardize}}).
...
fit1 <- lm(price ~ carat + cut + depth + color + table + clarity, data=diamonds)
# Misleading result
