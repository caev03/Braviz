...
class BravizConfig(RawConfigParser):
    ...
    def get_interaction_style(self):
        ...
        try:
            ...
            log.error('Erroneous interactor_style value %s' %
                      custom_interactor_style)
            raise Exception(
                'Erroneous interactor_style value %s' % custom_interactor_style)
        style = vtk_attrs[idx]
        return style
...
def make_default_config(default_config_name=None):
    ...
    braviz_conf.set(
        'Default_Variables', 'laterality', 'LAT_EdinburgHandedness')
    ...
    try:
        ...
        log.info("default configuration file created in %s" %
                 default_config_name)
    except IOError:
        log.error("couldn't create default configuration file in %s" %
                  default_config_name)
