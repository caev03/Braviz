...
class ExportScalarToDataBase(QtGui.QDialog):
    ...
    def setupUI(self):
        ...
        if self.fibers_mode is False:
            ...
            if self.fibers_operation is not None:
                label = "Fibers passing through " + \
                    self.fibers_operation.join(self.structs)
    ...
    def start_calculations(self):
        ...
        if potential_conflict is None:
            try:
                new_index = braviz_tab_data.register_new_variable(
                    self.var_name, is_real=True)
            except Exception:
                potential_conflict = True
        if potential_conflict is not None:
            self.ui.error_str.setText(
                "Variable name exists, pleas try with a different name")
        ...
        self.ui.tip_label.setText(
            "Note: You can minimize this dialog and continue working")

        # update scenario
        if self.scenario_id is not None:
            name = "<AUTO_%s>" % self.var_name
            description = "Created automatically when recording variable %s" % self.var_name
            braviz_user_data.update_scenario(
                self.scenario_id, name=name, description=description)
            # link scenario
        ...
        self.progress_thread = threading.Thread(
            target=self.save_variables_function)
    ...
    def get_scalar_value(self, subj):
        image_code = subj
        if self.fibers_mode is False:
            val = braviz_struct_metrics.get_mult_struct_metric(
                self.reader, self.structs, image_code, self.metric_code)
            return val
        else:
...
def run(fibers=False, structures_list=tuple(), metric="volume", db_id=None, operation="and", scenario_id=None):
    ...
    main_window = ExportScalarToDataBase(
        fibers, structures_list, metric, db_id, operation, scenario_id=scenario_id)
    main_window.show()
    log = logging.getLogger(__name__)
    try:
        app.exec_()
...
if __name__ == "__main__":
    ...
    if not fibers:
        ...
        structs = args[4:]
        log.info(structs)
        run(fibers=fibers, structures_list=structs,
            metric=metric, scenario_id=scenario_id)
    else:
        # Fibers
        operation = args[4]
        db_id = args[5]
