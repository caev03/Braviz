    ...
    def __init__(self):
        ...
        def changeReference(new_ref):
            ...
            if active_planes.get():
                img1 = reader.get(
                    'MRI', ref_subj, format='VTK', space=curr_space)
        ...
        # Sin interpolar
        planeWidget2.SetResliceInterpolateToNearestNeighbour()
        ...
        def addModel(model_name, addToRef=True):
            ...
            availableModels = reader.get('model', ref_subj, index='t')
            if addToRef:
            ...
            if models_dict.has_key(model_name):
                ...
                if model_name in availableModels:
                    model = reader.get(
                        'MODEL', subj, name=model_name, space=curr_space)
                ...
                if model_name in availableModels:
                    model = reader.get(
                        'MODEL', subj, name=model_name, space=curr_space)
                    model_color = reader.get(
                        'MODEL', None, name=model_name, color='T')
            ...
            if addToRef:
                model_volume = reader.get(
                    'model', ref_subj, name=model_name, volume='1')
                add_solid_balloon(
                    balloon, models_dict[model_name][2], model_name, model_volume)
            else:
                #"show personalized message"
                volume_other = reader.get(
                    'model', curr_subj, name=model_name, volume='1')
                _, area_r = braviz.interaction.compute_volume_and_area(
                    ref_models[model_name][0])
                _, area_o = braviz.interaction.compute_volume_and_area(
                    other_models[model_name][0])
                vol_o = reader.get(
                    'model', curr_subj, name=model_name, volume='1')
                vol_r = reader.get(
                    'model', ref_subj, name=model_name, volume='1')
                ...
                message = "%s\nVolume* = %.2f mm3 ( %+.2f )\nSurface Area = %.2f mm2 ( %+.2f )" % (
                    model_name, vol_o, vol_d, area_o, area_d)
                if dual_disp:
        ...
        def update_balloons():
            ...
            for model_name in other_models.iterkeys():
                vol_r, area_r = braviz.interaction.compute_volume_and_area(
                    ref_models[model_name][0])
                vol_o, area_o = braviz.interaction.compute_volume_and_area(
                    other_models[model_name][0])
                vol_d = vol_o - vol_r
                area_d = area_o - area_r
                message = "%s\nVolume = %.2f mm3 ( %+.2f )\nSurface Area = %.2f mm2 ( %+.2f )" % (
        ...
        def async_read_fibers():
            ...
            fibers_t = reader.get(
                'Fibers', ref_subj, waypoint=waypoints, space=curr_space, color=color_var.get())
            progress_internal.set(50)
            fibers2_t = reader.get(
                'Fibers', curr_subj, waypoint=waypoints, space=curr_space, color=color_var.get())
        ...
        Mean Length: %.2f
            ...
            if descriptors_cache.has_key(other_key):
                ...
                d_o = braviz.interaction.get_fiber_bundle_descriptors(
                    fibers2_t)
        ...
        Mean Length (mm) : %.2f ( %+.2f )
            ...
            if dual_disp:
                ...
                balloon.RemoveBalloon(fibers_actor2)
                # To have it also in split view
                balloon.AddBalloon(fibers_actor2, custom_message)
        ...
        def change_subj(event):
            ...
            if active_planes.get():
                if dual_disp:
                    img2 = reader.get(
                        'MRI', curr_subj, format='vtk', space=curr_space)
                    planeWidget2.SetInputData(img2)
        ...
        subjects = braviz.interaction.subjects_list(
            reader, change_subj, control_frame)
        subjects.grid(row=0)
        ...
        ref_label = tk.Label(
            ref_frame, text=' Reference: ', justify='left', border=1)
        ...
        reference_button = tk.Button(
            control_frame, text='Set Reference', command=refButton)
        ...
        plane_button = tk.Button(
            control_frame, text='Activate interactive plane', command=activate_interactive_plane)
        ...
        show_planes = tk.Checkbutton(
            images_structs_frame, text='Images', command=imagePlanesStatus, variable=active_planes, pady=10)
        show_planes.grid(row=0, column=0, sticky='ew')

        show_structs = tk.Checkbutton(
            images_structs_frame, text='Waypoints', command=visibleStructsStatus, variable=active_waypoints, pady=10)
        ...
        select_model = braviz.interaction.structureList(
            reader, ref_subj, changeStruct, control_frame, waypoints, text='Waypoints', padx=10, pady=15)
        select_model.grid(row=20, sticky='WESN', pady=5)
        control_frame.rowconfigure(20, weight=1)

        calculate_but = tk.Button(
            control_frame, text="Calculate fibers", command=calculate_fibers)
        ...
        fibers_progress_bar = ttk.Progressbar(
            control_frame, orient='horizontal', length='100', mode='determinate', variable=progress)
        ...
        planeWidget2.AddObserver(
            vtk.vtkCommand.AnyEvent, replicateCursor, 1000)
        ...
        def move_sphere(obj, event):
            ...
            displacement = [
                x - y for x, y in izip(new_pos, initial_sphere_pos)]
        ...
        ref_plane_Widget.AddObserver(
            vtk.vtkCommand.InteractionEvent, copyPlane, 10)
        ref_plane_Widget2.AddObserver(
            vtk.vtkCommand.InteractionEvent, copyPlane, 10)

        widgets = [select_model, plane_button, show_planes, show_structs,
                   reference_button, ref_input, subjects, calculate_but]
