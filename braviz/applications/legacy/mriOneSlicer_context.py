...
from braviz.visualization.simple_vtk import add_solid_balloon, add_fibers_balloon, persistentImagePlane
...
if __name__ == "__main__":
    ...
    def setModel(event=None):
        ...
        for model_name in requested:
            poly_data, mapper, actor = models.get(
                model_name, (None, None, None))
            poly_data = reader.get(
                'MODEL', currSubj, name=model_name, space=currSpace)
            if mapper is None:
            ...
            model_properties.SetColor(list(model_color[0:3]))
            volume_model = reader.get(
                'model', currSubj, name=model_name, volume=1)
    ...
    def show_fibers():
        ...
        if fibers_active.get():
            ...
            thread.start_new_thread(
                async_load_fibers, (model_list.get(), and_or_variable.get()))
    ...
    def async_load_fibers(models, operation):
        ...
        fibers = reader.get(
            'fibers', currSubj, waypoint=models, space=currSpace, operation=operation)
    ...
    #================================CONTEXT==========================
    context_models = ['3rd-Ventricle', '4th-Ventricle', '5th-Ventricle', 'Brain-Stem', 'CSF',
                      'Left-Lateral-Ventricle', 'Left-vessel', 'Right-Lateral-Ventricle', 'Right-vessel']
    context_dict = {}

    def update_context():
        for m in context_dict.iterkeys():
            c_model, c_map, c_actor = context_dict[m]
            c_model = c_model = reader.get(
                'model', currSubj, space=currSpace, name=m)
    ...
    fibers_check = tk.Checkbutton(
        fibers_frame, text='Fibers', command=show_fibers, variable=fibers_active)
    ...
    show_planes = tk.Checkbutton(
        fibers_frame, text='Plane', command=imagePlanesStatus, variable=active_planes, pady=10)
    show_planes.grid(column=0, row=0)
    fibers_check.grid(column=1, row=0)
    and_or_variable = tk.StringVar()
    and_or_variable.set('or')
    and_or_combo = ttk.Combobox(fibers_frame, textvariable=and_or_variable, values=[
    ...
    model_list = braviz.interaction.tk_gui.structureList(
        reader, currSubj, None, control_frame)
    ...
    class ctx_dialog(simpleDialog):
        ...
        def body(self, master):
            label = tk.Label(
                master, text='Select structures to show as context')
            label.grid(row=0, sticky='ew')
            self.select_model = braviz.interaction.structureList(
                reader, currSubj, None, master, initial_models=context_models, text='Model', padx=10, pady=5)
    ...
    apply_but = tk.Button(
        control_frame, text='Apply Selection', command=setModel, pady=0)
    apply_but.pack(side='top', fill='x', expand=0, pady=1)

    ctx_but = tk.Button(
        control_frame, text='Add Context', command=ctx_action, pady=0)
    ctx_but.pack(side='top', fill='x', expand=0, pady=1)

    widgets = [model_list, fibers_check, select_subj_frame,
               show_planes, ctx_but, apply_but, model_list]
    ...
    distance_w.AddObserver(
        vtk.vtkCommand.StartInteractionEvent, show_measure_tip)
    ...
    def translateMeasure(previous_space, new_space):
        ...
        poly_data2 = reader.transform_points_to_space(
            poly_data, previous_space, currSubj, True)
        # Poly Data 2 is in world coordinates
        poly_data3 = reader.transform_points_to_space(
            poly_data2, new_space, currSubj, False)
