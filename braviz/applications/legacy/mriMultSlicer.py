...
class MriMultSlicerApp(object):
    ...
    def __init__(self, pipe=None):
        ...
        self.img = reader.get(
            'MRI', self.currSubj, format='VTK', space=self.currSpace)
        aparc = reader.get(
            'aparc', self.currSubj, format='vtk', space=self.currSpace)
        ...
        def setSubj(event=None):
            ...
            try:
                self.img = reader.get(
                    images_var.get(), subj, format='VTK', space=self.currSpace)
                aparc = reader.get(
                    'aparc', subj, format='vtk', space=self.currSpace)
            except Exception:
                self.img = None
                aparc = None
        ...
        def addModel(model_name):
            ...
            if models.has_key(model_name):
                ...
                if model_name in self.availableModels:
                    model = reader.get(
                        'MODEL', self.currSubj, name=model_name, space=self.currSpace)
            ...
            model_volume = reader.get(
                'model', self.currSubj, name=model_name, volume=1)
        ...
        def add_tracts():
            ...
            try:
                tracts = reader.get(
                    'fibers', self.currSubj, space=space_var.get(), waypoint=models, operation='or')
            except Exception:
                tracts = None
        ...
        def picking(caller, event, *args):
            ...
            if self.highlighted_item:
                select_model_frame.itemconfigure(
                    self.highlighted_item, selectbackground='')
            ...
            if picked and picked_model:
                ...
                select_model_frame.itemconfigure(
                    self.highlighted_item, selectbackground='sea green')
        ...
        select_subj_frame = braviz.interaction.subjects_list(
            reader, subj_selection, control_frame, text='Subject', padx=10, pady=5, height='100')
        ...
        def subj_context_init(event):
            ...
            subj_context_menu.entryconfig(
                0, label='Set %s as context' % subject, command=lambda: set_context(subject_idx))
            subj_context_menu.tk_popup(event.x_root, event.y_root)

        def set_context(subject_idx):
            clear_context()
            self.ctxt_subject_idx = subject_idx
            select_subj_frame.subjects_list.itemconfigure(
                subject_idx, bg='light grey')
            subject = select_subj_frame.subjects_list.get(subject_idx)
        ...
        coordinates_label = tk.Label(
            control_frame, text='Coordinates:', pady=5)
        ...
        show_planes = tk.Checkbutton(
            planes_frame, text='Show Plane', command=imagePlanesStatus, variable=active_planes, pady=10)
        images_var = tk.StringVar()
        images_var.set('MRI')
        image_type = ttk.Combobox(
            planes_frame, textvariable=images_var, width=5)
        ...
        def change_img_type(event=None):
            ...
            if images_var.get() == 'MRI':
                ...
                self.img = reader.get(
                    'aparc', self.currSubj, format='vtk', space=self.currSpace)
                img_lut = reader.get(
                    'aparc', self.currSubj, format='vtk', space=self.currSpace, lut='t')
            else:
        ...
        def toggle_show_tracts(event=None):
            if show_tracts_var.get():
                ...
                tracts_actor.SetVisibility(0)
            refresh_display()
        show_tracts_toggle = tk.Checkbutton(
            control_frame, text='Show tracts', variable=show_tracts_var, command=toggle_show_tracts)
        show_tracts_toggle.pack(side='top')
        #===========================models list=======================
        select_model_frame = braviz.interaction.structureList(
            reader, self.currSubj, process_model, control_frame, text='Model', padx=10, pady=5)
        ...
        measure_tip.SetInput(
            "Right click on subject list to add context\nPress 'm' to measure")
        ...
        def translateMeasure(previous_space, new_space):
            ...
            poly_data2 = reader.transform_points_to_space(
                poly_data, previous_space, self.currSubj, True)
            # Poly Data 2 is in world coordinates
            poly_data3 = reader.transform_points_to_space(
                poly_data2, new_space, self.currSubj, False)
