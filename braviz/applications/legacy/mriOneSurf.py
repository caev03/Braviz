...
if __name__ == "__main__":
    ...
    props = {id(l_surf_actor): "Left Hemisphere", id(
        r_surf_actor): "Right Hemisphere"}
    ...
    def get_lut():
        ...
        # print scalar.get()
        out_lut = reader.get(
            'SURF_SCALAR', currSubj, scalars=scalar.get(), hemi='l', lut='t')
    ...
    def async_update():
        ...
        if la_internal:
            ...
            try:
                l_surf_t = reader.get(
                    'Surf', currSubj, name=surface.get(), hemi='l', scalars=scalar.get())
            except Exception:
                l_surf_t = None
        # time.sleep(10)
        progress_lock.acquire()
    ...
    select_subj_frame = braviz.interaction.subjects_list(
        reader, setSubj, control_frame, text='Subject', padx=10, pady=5, height='100')
    ...
    left_check = tk.Checkbutton(
        hemi_frame, text='left', variable=left_active, command=left_command)
    right_check = tk.Checkbutton(
        hemi_frame, text='right', variable=right_active, command=right_command)
    ...
    repr_frame = tk.Frame(
        control_frame, padx=20, pady=20, borderwidth=1, relief='groove')
    repr_frame.columnconfigure(0, weight=1)
    repr_frame.columnconfigure(1, weight=1)
    surf_label = tk.Label(repr_frame, text='Surface:')
    pial_radio = tk.Radiobutton(
        repr_frame, text='Pial', variable=surface, value='pial', command=update)
    white_radio = tk.Radiobutton(
        repr_frame, text='White', variable=surface, value='white', command=update)
    orig_radio = tk.Radiobutton(
        repr_frame, text='Orig', variable=surface, value='orig', command=update)
    inflated_radio = tk.Radiobutton(
        repr_frame, text='Inflated', variable=surface, value='inflated', command=update)
    sphere_radio = tk.Radiobutton(
        repr_frame, text='Sphere', variable=surface, value='sphere', command=update)
    ...
    curv_radio = tk.Radiobutton(
        repr_frame, text='Curv', variable=scalar, value='curv', command=update)
    area_radio = tk.Radiobutton(
        repr_frame, text='Area', variable=scalar, value='area', command=update)
    thickness_radio = tk.Radiobutton(
        repr_frame, text='Thickness', variable=scalar, value='thickness', command=update)
    volume_radio = tk.Radiobutton(
        repr_frame, text='Volume', variable=scalar, value='volume', command=update)
    sulc_radio = tk.Radiobutton(
        repr_frame, text='Sulc', variable=scalar, value='sulc', command=update)
    avg_radio = tk.Radiobutton(
        repr_frame, text='Avg curv', variable=scalar, value='avg_curv', command=update)
    aparc_radio = tk.Radiobutton(
        repr_frame, text='Parcellation', variable=scalar, value='aparc', command=update)
    aparc2009_radio = tk.Radiobutton(
        repr_frame, text='Parcellation 2009', variable=scalar, value='aparc.a2009s', command=update)
    ba_radio = tk.Radiobutton(
        repr_frame, text='Broadman area', variable=scalar, value='BA', command=update)
    ...
    mouse_press_event_id = iact.AddObserver(
        vtk.vtkCommand.LeftButtonPressEvent, picking, 10)
