...
if __name__ == "__main__":
    ...
    group_stats_dict = {}
    context_lines = [
        term_mean + term_std_dev, term_mean, term_mean - term_std_dev]
    ...
    # context
    brain_stem = reader.get(
        'model', current_subject, space='talairach', name='Brain-Stem')
    ...
    precentral = reader.get(
        'model', current_subject, space='talairach', name='ctx-lh-precentral')
    ...
    text_property.ShadowOn()
    number_of_fibers_message.GetPositionCoordinate(
    ).SetCoordinateSystemToNormalizedViewport()
    number_of_fibers_message.GetPositionCoordinate().SetValue(
        0.01, 0.5)  # number of fibers message
    ...
    text_property.SetColor(0, 0, 0)
    text_property.ShadowOn()
    orientation_message.GetPositionCoordinate(
    ).SetCoordinateSystemToNormalizedViewport()
    ...
    def get_img(subject, img_type, side='r'):
        ...
        if img_type == 'cc':
            ...
            try:
                ...
                fibers = reader.get(
                    'fibers', subject, space='talairach', name='cortico_spinal_%c' % side)
                brain_stem = reader.get(
                    'model', current_subject, space='talairach', name='Brain-Stem')
                precentral = reader.get(
                    'model', current_subject, space='talairach', name='ctx-%ch-precentral' % side)
            except Exception:
                fibers_actor.SetVisibility(0)
                precentral_actor.SetVisibility(0)
                brain_stem_actor.SetVisibility(0)
        ...
        if fibers_actor.GetVisibility():
            ...
            number_of_fibers_message.SetInput(
    ...
    def set_data(event=None):
        ...
        if len(group_dict) == 0:
            ...
            grupo = get_column(csv_file, 'UBICA')
        ...
        valid_genres = set()
        if male_selected_var.get():
            valid_genres.add('2')
        if female_selected_var.get():
            valid_genres.add('1')
        ...
        if len(genre_codes) > 0:
            codes2 = genre_codes
            tms_data2 = map(
                lambda x: tms_data_dict.get(x, float('Nan')), codes2)
        else:
            codes2 = []
            tms_data2 = []
        ...
        bars_view1.set_color_fun(
            lambda x: get_color(x, color_inversion_factor))
        ...
        if show_groups_var.get() is True:
            for i, cod in enumerate(codes2):
                select_subj_frame.itemconfigure(
                    i, background=group_colors_dict[group_dict[cod]])
        else:
            for i, cod in enumerate(codes2):
    ...
    def draw_bars_1():
        if showing_history is True:
            ...
            if not show_groups_var.get():
                ...
                group_stats_dict.update(
                    zip(['KMC', 'INCUB', 'TERM'], zip(means, stds)))
                bars_view1.set_higlight_index(None)
    ...
    def set_subj(event=None):
        ...
        if animation is False:
            ...
            if animated_draw_bar_id is not None:
                root.after_cancel(animated_draw_bar_id)
    ...
    def animated_draw_bar(time, slope, value):
        ...
        if time > 0:
            animated_draw_bar_id = root.after(
                50, animated_draw_bar, time - 1, slope, value + slope)
    ...
    data_selection_tree = ttk.Treeview(
        select_data_frame, show='tree', height=8, selectmode='browse')
    data_selection_tree.insert(
        '', 'end', 'motor_brain', text='Motor Brain', tags='parent')
    data_selection_tree.insert(
        'motor_brain', 'end', 'exci', text='Excitability', tags='leaf')
    data_selection_tree.insert(
        'motor_brain', 'end', 'sync', text='Synchronization', tags='leaf')
    data_selection_tree.insert(
        'motor_brain', 'end', 'balan', text='Balanced Activity', tags='parent')
    data_selection_tree.insert(
        'balan', 'end', 'inhi', text='Level of Inhibition', tags='leaf')
    data_selection_tree.insert(
        'balan', 'end', 'faci', text='Level of Facilitation', tags='leaf')
    data_selection_tree.insert(
        'motor_brain', 'end', 'coop', text='Cooperation between hemispheres', tags='parent')
    data_selection_tree.insert(
        'coop', 'end', 'freq', text='Frequency', tags='leaf')
    data_selection_tree.insert(
        'coop', 'end', 'trans', text='Transfer time', tags='leaf')
    data_selection_tree.insert(
        'coop', 'end', 'dura', text='Duration', tags='leaf')
    ...
    data_tree_tooltip = ToolTip(
        data_selection_tree, msgFunc=data_tree_message_func, follow=1, delay=0.5)
    ...
    dominant_radio = tk.Radiobutton(
        select_data_frame, text='Dominant', variable=side_var, value='d', command=set_data)
    ...
    male_checkbox = tk.Checkbutton(
        select_data_frame, text='males', command=set_data, variable=male_selected_var)
    female_checkbox = tk.Checkbutton(
        select_data_frame, text='females', command=set_data, variable=female_selected_var)
    ...
    show_groups_box = tk.Checkbutton(
        select_data_frame, text='Show groups', variable=show_groups_var, command=set_data)
    ...
    show_all_or_history_button = tk.Button(
        control_frame, text='Show all', command=show_all_or_history)
    ...
    add_to_hist_button = tk.Button(
        control_frame, text='Add to history <----', command=add_to_history)
    ...
    remove_from_hist_button = tk.Button(
        control_frame, text='Remove from history', command=remove_from_history)
    ...
    bars_1_tooltip = ToolTip(
        bars_widget1, msgFunc=bars_1_msg_func, follow=1, delay=0.5)
    ...
    bars_2_tooltip = ToolTip(
        bars_widget2, msgFunc=bars_2_msg_func, follow=1, delay=0.5)
